...
namespace big
{
	...
	rage::eThreadState hooks::gta_thread_tick(GtaThread* thread, unsigned int a2)
	{
		...
		if (thread->m_script_hash == RAGE_JOAAT("freemode") && state == rage::eThreadState::running && !g_settings.protections.freemode_terminated)
		{
		...
		{
			if (result == rage::eThreadState::killed && state == rage::eThreadState::running && !g_settings.protections.freemode_terminated)
			{
			...
			if (g_settings.protections.freemode_terminated) g_settings.protections.freemode_terminated = !(result == rage::eThreadState::running);
		}
