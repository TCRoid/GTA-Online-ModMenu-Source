...
{
	std::vector<Vehicle>lastVehicle;
	std::vector<Object>lastObject;
	std::vector<Ped>lastPed;
	void deleteLastEntity()
	{
		if (g_settings.options["spwancardel_mod"]) {
			for (Vehicle vehicle : lastVehicle)
			{
				ENTITY::SET_ENTITY_AS_MISSION_ENTITY(vehicle, true, true);
				VEHICLE::DELETE_VEHICLE(&vehicle);
			}
			for (Object object : lastObject)
			{
			...
			g_gui.SendNotification("INI载具", StringToChar("已经删除" + std::to_string(lastVehicle.size() + lastObject.size() + lastPed.size()) + "个模型"));
			lastVehicle.clear();
			lastObject.clear();
			lastPed.clear();
		}
	...
	void createXmlVehicle(const char* xml)
	{
		...
		*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x9090;
		vehicle.e = features::spawnVehicle(VehicleModel);
		*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x0574;
		lastVehicle.push_back(vehicle.e);
		...
		//设置引力
		bool HasGravity = nodeVehicle.child("HasGravity").text().as_bool(true);
		VEHICLE::SET_VEHICLE_GRAVITY(vehicle.e, HasGravity);
		ENTITY::SET_ENTITY_HAS_GRAVITY(vehicle.e, HasGravity);
		...
		for (auto& nodeAttachment = nodeAttachments.first_child(); nodeAttachment; nodeAttachment = nodeAttachment.next_sibling())
		{
			...
			{
				*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x9090;
				e.e = OBJECT::CREATE_OBJECT(eModel, placingEpos.x, placingEpos.y, placingEpos.z, true, true, Dynamic);
				*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x0574;
			}
			else if (Type == 1)
			{
	...
	bool createIniVehicle(const char* vehiclename)
	{
		...
		for (int i = 0; i < allVehicleCount && entityList[i].type == 0; i++)
		{
			...
			*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x9090;
			lastVehicle.push_back(entityList[i].e = VEHICLE::CREATE_VEHICLE(entityList[i].hash, ourCoords.x + u(e), ourCoords.y + u(e), ourCoords.z + u(e), ENTITY::GET_ENTITY_HEADING(PLAYER::PLAYER_PED_ID()), 1, 0, 1));
			*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x0574;
			util::requestControlOfEnt(entityList[i].e);
		...
		for (int i = 0; i < allObjectsCount && entityList[i].type == 1; i++)
		{
			...
			*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x9090;
			lastObject.push_back(entityList[i].e = OBJECT::CREATE_OBJECT(entityList[i].hash, ourCoords.x + u(e), ourCoords.y + u(e), ourCoords.z + u(e), 1, 1, INI::GetBool(vehiclename, ("Object" + std::to_string(i)).c_str(), "Dynamic")));
			*(unsigned short*)g_pointers->m_model_spawn_bypass = 0x0574;
			util::requestControlOfEnt(entityList[i].e);
