...
functions::NetEventType7* OG_SCRIPTEDGAME_EVENT = nullptr;
bool HK_SCRIPTEDGAME_HANDLER(CScriptedGameEvent* thisptr, __int64 player, __int64 receiver) {
	auto args = thisptr->m_args;
	if (args[0] == -1386010354)
	{
		LOG(INFO) << "脚本事件崩溃保护 | 来自玩家:" << PLAYER::GET_PLAYER_NAME(args[1]);
		return true;
	}
	else if (args[0] == 998716537)
	{
		LOG(INFO) << "脚本事件踢出保护 | 来自玩家:" << PLAYER::GET_PLAYER_NAME(args[1]);
		return true;
	}
	else if (args[0] == 163598572)
	{
		LOG(INFO) << "脚本事件踢出保护 | 来自玩家:" << PLAYER::GET_PLAYER_NAME(args[1]);
		return true;
	}
	return OG_SCRIPTEDGAME_EVENT(thisptr, player, receiver);
}
...

{
	...
	MH_CreateHook(g_pointers->m_get_event_data, g_hooking->HK_GET_EVENT_DATA, (LPVOID*)&o_getEventData);

	MH_CreateHook((LPVOID)g_pointers->m_get_scriptedgame_events, (LPVOID)HK_SCRIPTEDGAME_HANDLER, (void**)&OG_SCRIPTEDGAME_EVENT);
...
void hooking::enable()
{
	MH_EnableHook(g_pointers->m_get_event_data);
	MH_EnableHook(g_pointers->m_get_scriptedgame_events);
...
void hooking::disable()
{
	...
	MH_DisableHook(g_pointers->m_get_scriptedgame_events);
	MH_RemoveHook(g_pointers->m_get_scriptedgame_events);

	MH_DisableHook(g_pointers->m_get_event_data);
	MH_RemoveHook(g_pointers->m_get_event_data);
...
BOOL hooking::HK_GET_EVENT_DATA(std::int32_t eventGroup, std::int32_t eventIndex, std::int64_t* args, std::uint32_t argCount)
{
	return g_hooking->o_getEventData(eventGroup, eventIndex, args, argCount);
}