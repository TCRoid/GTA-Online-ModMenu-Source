...
void native_invoker::end_call(rage::scrNativeHash hash)
{
	...
	if (auto it = m_handler_cache.find(hash); it != m_handler_cache.end())
	{
		...
		if (handler != nullptr) {
			...
			__try
			{
				_call_asm(reinterpret_cast<void*>(&m_call_context), handler, g_pointers->m_nativeReturn); // Native Spoof
				g_pointers->m_fix_vectors(&m_call_context); // "Fix" Vectors
			}
			__except (exceptionadd = (GetExceptionInformation())->ExceptionRecord->ExceptionAddress, EXCEPTION_EXECUTE_HANDLER)
			{
				[hash]() { LOG(INFO) << "Failed to invoke native " << HEX_TO_UPPER(hash); }();
			}
		}
	}
	else
	{
		[hash]() { LOG(INFO) << "Failed to find " << HEX_TO_UPPER(hash) << " native's handler."; }();
	}
	*/
	if (auto it = m_handler_cache.find(hash); it != m_handler_cache.end())
	{
