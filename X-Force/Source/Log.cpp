...
void Log::Init(HMODULE hModule) {
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0) {
		...
		sprintf_s(chLogBuff, szTimestamp, "INIT: X-Force");
		Menu::addText("[" + std::to_string(current_tm.tm_hour) + ":" + std::to_string(current_tm.tm_min) + ":" + std::to_string(current_tm.tm_sec) + "] " + "INIT: X-Force", true);
		if (slash != -1) {
			g_logFile[slash + 1] = '\0';
			strcpy_s(g_debugLogFile, g_logFile);
			strcat_s(g_debugLogFile, "X-Log.log");
			...
			OutputDebugStringA("INIT: X-Force");
		}
		else {
			useFileOutput = false;
		}
