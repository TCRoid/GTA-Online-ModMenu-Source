...
Vehicle create_veh(Player plr, Hash hash) {
	...
	Vehicle veh = VEHICLE::CREATE_VEHICLE(hash, pos.x, pos.y + 5.f, pos.z, ENTITY::GET_ENTITY_HEADING(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(plr)), 1, 0);
...
Object create_obj(Player plr, Hash hash) {
	STREAMING::REQUEST_MODEL(hash);
	Object obj = OBJECT::CREATE_OBJECT(hash, pos.x, pos.y, pos.z, 1, 1, 0);
	return obj;
}

void gasCylinderMK2(Player plr) {
...
void moddedvehBigOrangeDick(Player plr) {
...
void moddedVeh6x6Monster(int plr) {
	//Vector3 MyCoords = ENTITY::GET_ENTITY_COORDS(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(selectedPlayer), true);
